...
params <- data.frame(id =learner.bb$param_set$ids(), class = unname(learner.bb$param_set$class), lower = unname(learner.bb$param_set$lower), unname(learner.bb$param_set$upper), levels = param_set$levels, default = param_set$default)
...
```{r}
library(gridExtra)
train_performance <- autoplot(benchmark, measure = measures[[1]])
test_performance <- autoplot(benchmark, measure = measures[[2]])
