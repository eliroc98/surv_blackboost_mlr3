...
The aim of this project is to show how to use mlr3's tool for classification tree's gradient boosting on a survival analysis task; for this purpose we will use the German Breast Cancer Study (GBCS) Dataset from Hosmer et al.  
...
<<<<<<< HEAD
In order to use this dataset in our next processes, we need to split it into train set and test set.
```{r}
set.seed(123)
...
Then, we need to build tasks which will be used during the training, testing and hyperparameters tuning procedures which we will discuss later.
=======
...
>>>>>>> 43b276580c2bb8e057a9f11753336dbffeedd35e
```{r}
...
```
___
We can now access the resulting optimized parameters in the `instance` and assign them to our new (optimized) learner:
```{r, results='hide'}
...
___
First, we create our benchmark model, which will be a classical Cox model.
```{r}
learner.cox = lrn("surv.coxph",id="cox",predict_sets=c("train","test"))
```

___
We define the triplet of objects that will make up the design.
```{r}
